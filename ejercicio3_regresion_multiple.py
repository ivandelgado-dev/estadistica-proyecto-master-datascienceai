...
def regresion_lineal_multiple(X_train, y_train, X_test):
    ...
    """
    # añadimos columna de unos para que el modelo pueda calcular el intercepto β₀
    unos = np.ones((X_train.shape[0], 1))
    X_b = np.hstack([unos, X_train])

    # resolvemos el sistema con lstsq, más estable numéricamente que invertir la matriz
    coefs, _, _, _ = np.linalg.lstsq(X_b, y_train, rcond=None)

    # hacemos lo mismo con X_test para poder multiplicar por los coeficientes
...
def graficar_real_vs_predicho(y_real, y_pred, ruta_salida="output/ej3_predicciones.png"):
    ...
    ax.scatter(y_real, y_pred, alpha=0.6, color="steelblue", edgecolors="white")

    # línea de referencia perfecta y=x
    minimo = min(y_real.min(), y_pred.min())
    maximo = max(y_real.max(), y_pred.max())
    ax.plot([minimo, maximo], [minimo, maximo], color="red", linewidth=1.5, linestyle="--")
