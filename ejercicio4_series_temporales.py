...
def visualizar_serie(serie):
    ...
    """
    # creamos el gráfico con tamaño amplio para ver bien la evolución temporal
    fig, ax = plt.subplots(figsize=(14, 4))
    ax.plot(serie, color="steelblue", linewidth=0.8)
    ax.set_title("Serie Temporal — World Happiness (sintética)")
    ax.set_xlabel("Fecha")
    ax.set_ylabel("Valor")
...
def descomponer_serie(serie):
    ...
    # descomposición aditiva con periodo de 365 días (un año)
    resultado = seasonal_decompose(serie, model="additive", period=365)
    
    # resultado.plot() genera automáticamente los 4 subgráficos (serie, tendencia, estacionalidad, residuo)
    fig = resultado.plot()
    fig.set_size_inches(14, 10)
    fig.suptitle("Descomposición de la serie temporal", fontsize=13)
    plt.tight_layout()
...
def analizar_residuo(residuo):
    ...
    # eliminamos NaN que genera la descomposición en los extremos
    residuo_limpio = residuo.dropna()

    # estadísticos básicos del residuo
    ...
    # test ADF para comprobar si el residuo es estacionario
    ...
    # ACF mide correlación con retardos anteriores
    # PACF mide la correlación directa eliminando el efecto de retardos intermedios
    ...
    # comparamos la distribución real del residuo con una normal teórica
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.hist(residuo_limpio, bins=50, density=True,
            color="steelblue", edgecolor="white", alpha=0.7)
    x = np.linspace(residuo_limpio.min(), residuo_limpio.max(), 200)
    ax.plot(x, norm.pdf(x, media, std), color="red", linewidth=2, label="Normal teórica")
    ax.set_title("Histograma del residuo con curva normal")
    ...
    with open("output/ej4_analisis.txt", "w", encoding="utf-8") as f:
        f.write("Análisis del residuo — Serie Temporal\n")
        f.write("=" * 45 + "\n")
        ...
        f.write(f"\nTest Jarque-Bera:\n")
        f.write(f"  Estadístico: {stat_jb:.6f}\n")
        f.write(f"  p-value: {p_jb:.6f}\n")
        f.write(f"\nTest ADF (estacionariedad):\n")
        f.write(f"  Estadístico: {resultado_adf[0]:.6f}\n")
